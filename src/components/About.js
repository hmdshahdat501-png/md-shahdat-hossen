...
export default function About() {
  ...
  if (!mounted) return (
    <section id="about" className="py-16 px-6 md:px-24 max-w-7xl mx-auto opacity-0" />
  );

  return (
    <section id="about" className="py-16 px-6 md:px-24 max-w-7xl mx-auto transition-colors duration-300">
      <div className="text-center mb-12">
        <h2 className="text-4xl md:text-5xl font-extrabold mb-4 reveal-about dark:text-white">About</h2>
        <p className="text-secondary dark:text-gray-400 text-sm uppercase tracking-widest font-bold">My Introduction</p>
      </div>
        ...
        <div className="text-left space-y-10">
            ...
            {stats.map((stat, i) => {
              ...
              >
                ...
                <div className="relative bg-zinc-50 dark:bg-zinc-900 p-5 rounded-[calc(1.5rem-1px)] h-full border border-zinc-200 dark:border-zinc-800 shadow-sm text-center space-y-2 z-10 transition-colors">
                  ...
                  <div className="font-bold text-xs uppercase tracking-tight text-zinc-900 dark:text-gray-100">{stat.label}</div>
                  <div className="text-[10px] text-zinc-500 dark:text-gray-400 font-medium">{stat.sub}</div>
                </div>
              </motion.div>
            )})}
          </div>
          ...
          <div className="space-y-6">
            ...
            >
              ...
              >
                <Download size={20} fill="currentColor" fillOpacity={0.2} strokeWidth={2.5} />
              </motion.div>
            </motion.button>
          </div>
        </div>
